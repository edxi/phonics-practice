              ...
              <div className="text-[40px] font-extrabold flex items-center">
                {word.phonicsUnits.map((u, i) => {
                  ...
                  return (
                    ...
                    >
                      {u.letters}
                      {i < word.phonicsUnits.length - 1 && i === Math.floor(word.phonicsUnits.length / 2) - 1 ? (
          ...
          {showDetails && (
            <div className="mt-3 p-3.5 bg-purple-50/70 rounded-2xl text-xs text-left text-slate-700 border border-purple-100 w-full animate-fadeIn space-y-2">
              ...
              <p className="text-slate-600 leading-relaxed">
                {word.detail || `自然拼读核心词汇：包含 ${word.phonicsUnits.length} 个音形对应音素。`}
              </p>
              {word.rootAffix && (
                <div className="pt-1.5 border-t border-purple-200/50">
                  <p className="font-bold text-purple-800 text-[11px] mb-0.5">词根助记：</p>
                  <p className="text-slate-600">
                    <span className="font-semibold text-amber-800">{word.rootAffix.root}</span> ({word.rootAffix.rootMeaning}) +{' '}
                    <span className="font-semibold text-purple-800">{word.rootAffix.affix}</span> ({word.rootAffix.affixMeaning}) ={' '}
                    <span className="font-bold text-slate-800">{word.rootAffix.combinedMeaning}</span>
                  </p>
                  {word.rootAffix.description && (
                    <p className="text-slate-500 text-[10px] mt-0.5">{word.rootAffix.description}</p>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="w-full mt-4">
            {breakdownMode === 'phonics' ? (
              ...
              <div className="flex items-center justify-center gap-2 overflow-x-auto py-1">
                {word.phonicsUnits.map((unit, idx) => {
          ...
          <div className="bg-white rounded-3xl p-4 shadow-xs border border-purple-50 relative">
            ...
            <div className="flex items-center justify-center gap-1.5 text-xs text-slate-700 mb-2">
              <div className="bg-amber-50 border border-amber-200/60 rounded-xl px-2.5 py-1.5 text-center min-w-[55px]">
                <div className="font-bold text-amber-700 text-sm">{word.rootAffix.root}</div>
                <div className="text-[11px] text-slate-500 mt-0.5">{word.rootAffix.rootMeaning}</div>
              </div>
              ...
              <div className="bg-purple-50 border border-purple-200/60 rounded-xl px-2.5 py-1.5 text-center min-w-[55px]">
                <div className="font-bold text-purple-700 text-sm">{word.rootAffix.affix}</div>
                <div className="text-[11px] text-slate-500 mt-0.5">{word.rootAffix.affixMeaning}</div>
              </div>
              <span className="font-bold text-slate-400 text-base">=</span>
              <div className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-center flex-1 max-w-[150px]">
                <div className="font-bold text-slate-800 text-sm">{word.word}</div>
                <div className="text-[11px] text-slate-500 truncate mt-0.5">{word.definition}</div>
              </div>
            </div>
        ...
        {word.spokenExample && (
          <div className="bg-white rounded-3xl p-4 shadow-xs border border-purple-50 relative">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={() => speechService.speakWord(word.spokenExample!.en)}
            ...
            <p className="text-sm font-semibold text-slate-800 mb-1">
              "{word.spokenExample.en}"
            </p>
            <p className="text-xs text-slate-500">
              {word.spokenExample.zh}
            </p>
          </div>
        )}
      ...
      <div className="px-4 pt-2 bg-[#f8f7fe] shrink-0 border-t border-purple-100/50">
        <div className="flex items-center gap-3">
          ...
          >
            <Star
              className={`w-6 h-6 ${
                word.isFavorite ? 'text-amber-400 fill-amber-400' : 'text-slate-400'
              }`}
            />
